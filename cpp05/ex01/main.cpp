...
static void testFormCreation(const std::string &name, int signGrade,
                             int execGrade) {
  std::cout << "\n[TEST] Creating Form(\"" << name << "\", sign " << signGrade
            << ", exec " << execGrade << ")\n";
  try {
...
static void testSigning(Bureaucrat &b, Form &f) {
  std::cout << "\n[TEST] " << b.getName() << " tries to sign " << f.getName()
            << std::endl;
...
int main() {
  ...
  try {
    ...
    std::cout << "Unexpected exception in signing scenarios: " << e.what()
              << std::endl;
  }
